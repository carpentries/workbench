...
the internal process that are invoked with the `sandpaper::build_lesson()`
function. This function has the effect of modifying the files inside of the
`site/` folder, which is the untracked folder that holds both the built markdown
cache and the rendered HTML site.
...
All of these methods will call `sandpaper::validate_lesson()` and the two-step
internal functions `sandpaper:::build_markdown()` and
`sandpaper:::build_site()`. Below, I break down and detail the process for each.
...
[{knitr}](https://yihui.org/knitr/) R package. Markdown content is simply
copied to the `site/built` folder. 
